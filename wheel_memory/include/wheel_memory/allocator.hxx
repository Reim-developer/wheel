...
WHEEL_MEMORY_NAMESPACE
    class Arena {
        private:
            struct Block {
                ...
                explicit Block(std::size_t capacity) : storage(capacity), offset(0) {}
                    ...
                    template<typename T>
                    T* try_allocate_array(size_t count) {
                        ...
                        if (!std::align(object_alignment, total_size, candidate, remaining_space)) {
                            return nullptr;
                        }
        ...
        public:
            ...
            [[nodiscard]] T* allocate(Args&& ...arguments) {
                ...
                T* allocate_array(size_t count) {
                    std::size_t total_size = sizeof(T) * count;
                    if (T *pointer = m_current_block->try_allocate_array<T>(count)) {
                        [[likely]]
                        return pointer;
                    }

                    return allocate_array_slow_path<T>(count);
                }
            #endif 
        ...
        private:
            ...
            #if defined (WHEEL_EXPERIMENT) && defined (WHEEL_SMALL_VEC)
                template<typename T>
                WHEEL_NO_INLINE T* allocate_array_slow_path(std::size_t count) {
                    auto current_iterator = current_block_iterator();
                    auto next_iterator              = std::next(current_iterator);

                    if (next_iterator != m_blocks.end()) {
                        if (T* pointer = next_iterator->try_allocate_array<T>(count)) {
                            m_current_block = &(*next_iterator);
                    ...
                    if (next_capacity < total_size + alignof(T)) {
                        next_capacity = std::bit_ceil(total_size  + alignof(T));
                    }

                    append_block(next_capacity);
                    T* pointer = m_current_block->try_allocate_array<T>(count);
