...
TEST(test_ast_function_declaration_creation)
    ...
    BlockStatement *func_body = arena.allocate<BlockStatement>(
        TOKEN(TokenKind::IDENT, arena), std::vector<StatementNode*>{}
    );
    FunctionDeclaration *func = arena.allocate<FunctionDeclaration>(
        TOKEN(TokenKind::IDENT, arena),
        func_name,
        params,
        func_body
    );
    ...
    assert_eq(func->token->kind, TokenKind::IDENT);
DONE 
...
TEST(test_ast_block_statement_creation)
    ...
    BlockStatement *block_stmt = arena.allocate<BlockStatement>(
        TOKEN(TokenKind::IDENT, arena), std::vector<StatementNode*>{stmt1, stmt2}
    );
    ...
    assert_eq(block_stmt->statements[0]->token->kind, TokenKind::IDENT);
    assert_eq(block_stmt->statements[1]->token->kind, TokenKind::IDENT);
    assert_eq(block_stmt->token->kind, TokenKind::IDENT);
DONE 
...
TEST(test_ast_nested_blocks)
    ...
    assert_eq(func_body->token->kind, TokenKind::IDENT);
DONE 

TEST(test_ast_variable_literal_declaration_creation)
    Arena arena;
    StringInterner interner;

    const auto var_name = interner.intern("my_variable");
    const auto var_type = interner.intern("int");

    LiteralExpression *literal = arena.allocate<LiteralExpression>(
        TOKEN(TokenKind::INT_LITERAL, arena)
    );
    VariableDeclaration *variable = arena.allocate<VariableDeclaration>(
        TOKEN(TokenKind::IDENT, arena),
        var_type, var_name,
        literal
    );

    assert_eq(variable->initializer->node_kind, NodeKind::LiteralExpression);
    assert_eq(variable->var_name, var_name);
    assert_eq(variable->var_type, var_type);
    assert_eq(variable->token->kind, TokenKind::IDENT);
    assert_eq(variable->initializer, literal);
    assert_eq(variable->initializer->token->kind, TokenKind::INT_LITERAL);
DONE 
...
TEST_MAIN
    ...
    RUN(test_ast_variable_literal_declaration_creation)
END_MAIN